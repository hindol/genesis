...
null;if(null!==number){if("le"===base||"be"===base)endian=base,base=10;this._init(number||0,base||10,endian||"be")}}function parseHex(str,start,end){var r=0;for(end=Math.min(str.length,end);start<end;start++){var c=str.charCodeAt(start)-48;r<<=4;r=49<=c&&54>=c?r|c-49+10:17<=c&&22>=c?r|c-17+10:r|c&15}return r}function parseBase(str,start,end,mul){var r=0;for(end=Math.min(str.length,end);start<end;start++){var c=str.charCodeAt(start)-48;r*=mul;r=49<=c?r+(c-49+10):17<=c?r+(c-17+10):r+c}return r}function smallMulTo(self,
num,out){out.negative=num.negative^self.negative;var len=self.length+num.length|0;out.length=len;len=len-1|0;var a=self.words[0]|0,b=num.words[0]|0;a*=b;var carry=a/67108864|0;out.words[0]=a&67108863;for(var k=1;k<len;k++){var ncarry=carry>>>26,rword=carry&67108863;carry=Math.min(k,num.length-1);for(var j=Math.max(0,k-self.length+1);j<=carry;j++)a=self.words[k-j|0]|0,b=num.words[j]|0,a=a*b+rword,ncarry+=a/67108864|0,rword=a&67108863;out.words[k]=rword|0;carry=ncarry|0}0!==carry?out.words[k]=carry|
0:out.length--;return out.strip()}function FFTM(x,y){this.x=x;this.y=y}function MPrime(name,p){this.name=name;this.p=new BN(p,16);this.n=this.p.bitLength();this.k=(new BN(1)).iushln(this.n).isub(this.p);this.tmp=this._tmp()}function K256(){MPrime.call(this,"k256","ffffffff ffffffff ffffffff ffffffff ffffffff ffffffff fffffffe fffffc2f")}function P224(){MPrime.call(this,"p224","ffffffff ffffffff ffffffff ffffffff 00000000 00000000 00000001")}function P192(){MPrime.call(this,"p192","ffffffff ffffffff ffffffff fffffffe ffffffff ffffffff")}
function P25519(){MPrime.call(this,"25519","7fffffffffffffff ffffffffffffffff ffffffffffffffff ffffffffffffffed")}function Red(m){"string"===typeof m?(m=BN._prime(m),this.m=m.p,this.prime=m):(assert(m.gtn(1),"modulus must be greater than 1"),this.m=m,this.prime=null)}function Mont(m){Red.call(this,m);this.shift=this.m.bitLength();0!==this.shift%26&&(this.shift+=26-this.shift%26);this.r=(new BN(1)).iushln(this.shift);this.r2=this.imod(this.r.sqr());this.rinv=this.r._invmp(this.m);this.minv=this.rinv.mul(this.r).isubn(1).div(this.m);
this.minv=this.minv.umod(this.r);this.minv=this.r.sub(this.minv)}"object"===typeof module?module.exports=BN:exports.BN=BN;BN.BN=BN;BN.wordSize=26;try{var Buffer=require("shadow$empty").Buffer}catch(e){}BN.isBN=function(num){return num instanceof BN?!0:null!==num&&"object"===typeof num&&num.constructor.wordSize===BN.wordSize&&Array.isArray(num.words)};BN.max=function(left,right){return 0<left.cmp(right)?left:right};BN.min=function(left,right){return 0>left.cmp(right)?left:right};BN.prototype._init=
function(number,base,endian){if("number"===typeof number)return this._initNumber(number,base,endian);if("object"===typeof number)return this._initArray(number,base,endian);"hex"===base&&(base=16);assert(base===(base|0)&&2<=base&&36>=base);number=number.toString().replace(/\s+/g,"");var start=0;"-"===number[0]&&start++;16===base?this._parseHex(number,start):this._parseBase(number,base,start);"-"===number[0]&&(this.negative=1);this.strip();"le"===endian&&this._initArray(this.toArray(),base,endian)};
BN.prototype._initNumber=function(number,base,endian){0>number&&(this.negative=1,number=-number);67108864>number?(this.words=[number&67108863],this.length=1):4503599627370496>number?(this.words=[number&67108863,number/67108864&67108863],this.length=2):(assert(9007199254740992>number),this.words=[number&67108863,number/67108864&67108863,1],this.length=3);"le"===endian&&this._initArray(this.toArray(),base,endian)};BN.prototype._initArray=function(number,base,endian){assert("number"===typeof number.length);
if(0>=number.length)return this.words=[0],this.length=1,this;this.length=Math.ceil(number.length/3);this.words=Array(this.length);for(base=0;base<this.length;base++)this.words[base]=0;var off=0;if("be"===endian)for(base=number.length-1,endian=0;0<=base;base-=3){var w=number[base]|number[base-1]<<8|number[base-2]<<16;this.words[endian]|=w<<off&67108863;this.words[endian+1]=w>>>26-off&67108863;off+=24;26<=off&&(off-=26,endian++)}else if("le"===endian)for(endian=base=0;base<number.length;base+=3)w=number[base]|
number[base+1]<<8|number[base+2]<<16,this.words[endian]|=w<<off&67108863,this.words[endian+1]=w>>>26-off&67108863,off+=24,26<=off&&(off-=26,endian++);return this.strip()};BN.prototype._parseHex=function(number,start){this.length=Math.ceil((number.length-start)/6);this.words=Array(this.length);for(var i=0;i<this.length;i++)this.words[i]=0;var j,off=0;i=number.length-6;for(j=0;i>=start;i-=6){var w=parseHex(number,i,i+6);this.words[j]|=w<<off&67108863;this.words[j+1]|=w>>>26-off&4194303;off+=24;26<=
off&&(off-=26,j++)}i+6!==start&&(w=parseHex(number,start,i+6),this.words[j]|=w<<off&67108863,this.words[j+1]|=w>>>26-off&4194303);this.strip()};BN.prototype._parseBase=function(number,base,start){this.words=[0];this.length=1;for(var limbLen=0,limbPow=1;67108863>=limbPow;limbPow*=base)limbLen++;limbLen--;limbPow=limbPow/base|0;var total=number.length-start,mod=total%limbLen;total=Math.min(total,total-mod)+start;for(var i=start;i<total;i+=limbLen)start=parseBase(number,i,i+limbLen,base),this.imuln(limbPow),
67108864>this.words[0]+start?this.words[0]+=start:this._iaddn(start);if(0!==mod){limbLen=1;start=parseBase(number,i,number.length,base);for(i=0;i<mod;i++)limbLen*=base;this.imuln(limbLen);67108864>this.words[0]+start?this.words[0]+=start:this._iaddn(start)}};BN.prototype.copy=function(dest){dest.words=Array(this.length);for(var i=0;i<this.length;i++)dest.words[i]=this.words[i];dest.length=this.length;dest.negative=this.negative;dest.red=this.red};BN.prototype.clone=function(){var r=new BN(null);this.copy(r);
...
b=a=a.isub(b).iushrn(this.shift);0<=a.cmp(this.m)?b=a.isub(this.m):0>a.cmpn(0)&&(b=a.iadd(this.m));return b._forceRed(this)};Mont.prototype.invm=function(a){return this.imod(a._invmp(this.m).mul(this.r2))._forceRed(this)}})("undefined"===typeof module$jscomp$0||module$jscomp$0,this)}
